...
public class TeamMemberRenderer implements IClientTeamRenderer, IEventHandler {
    ...
    public static final RenderType TEAM_MARKER_RENDER_TYPE = CustomRenderType.SolidTranslucentColor;
    ...
    public void onAfterTranslucentBlocks(IRenderLevelStageEvent event) {
        ...
        for (TeamMemberInfo member : teamData.teamMemberInfoList) {
            ...
            if (teammatePlayer != null) {
                // 渲染
                Vec3 currentPos = teammatePlayer.position();
                double posX = Mth.lerp(partialTicks, teammatePlayer.xOld, currentPos.x);
                double posY = Mth.lerp(partialTicks, teammatePlayer.yOld, currentPos.y);
                double posZ = Mth.lerp(partialTicks, teammatePlayer.zOld, currentPos.z);
