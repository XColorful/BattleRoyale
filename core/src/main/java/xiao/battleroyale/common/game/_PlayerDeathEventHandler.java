 ...
 */
public class _PlayerDeathEventHandler implements IEventHandler {

    private _PlayerDeathEventHandler() {}

    private static class PlayerEventHandlerHolder {
        private static final _PlayerDeathEventHandler INSTANCE = new _PlayerDeathEventHandler();
    }

    public static _PlayerDeathEventHandler get() {
    ...
    private void onLivingDeath(ILivingDeathEvent event) { // 接收被PlayerRevive取消的事件
        ...
        GamePlayer gamePlayer = _GameTeamManager.getGamePlayerByUUID(livingEntity.getUUID());
        if (gamePlayer == null) {
            return;
        }

        if (!_GameTeamManager.hasStandingGamePlayer(livingEntity.getUUID())) {
        ...
        if (event.isCanceled()) { // 被PlayerRevive取消
            ...
            gameManager.onPlayerDown(event, gamePlayer); // GameManager 只提供事件是否取消，下放 PlayerRevive 倒地机制判定及处理
        } else { // 死亡
