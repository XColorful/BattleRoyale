...
public class _StatsEventHandler {
    ...
    protected static void onGameStart(StatsManager statsManager, GameStartFinishEvent event) {
        ...
        // 清除内存数据 (不是记分板)
        _SpecialStatsEventHandler.onNewJourney(statsManager, gameManager);
        _SpecialStatsEventHandler.onNewMaxSpeed(statsManager, gameManager);
    ...
    protected static void onGameTick(StatsManager statsManager, GameTickFinishEvent event) {
        ...
        if (gameTickTime > statsManager.journeyStatsDelay) {
            _SpecialStatsEventHandler.onJourneyStats(statsManager, gameManager);
        }
        if (gameTickTime > statsManager.maxSpeedStatsDelay) {
            _SpecialStatsEventHandler.onMaxSpeedStats(statsManager, gameManager);
        }
    }
    ...
    protected static void onGameComplete(StatsManager statsManager, GameCompleteFinishEvent event) {
        ...
        if (statsManager.enableJourneyStats) {
            _SpecialStatsEventHandler.onJourneyComplete(statsManager, event);
        }
        if (statsManager.enableMaxSpeedStats) {
            _SpecialStatsEventHandler.onMaxSpeedComplete(statsManager, event);
        }
