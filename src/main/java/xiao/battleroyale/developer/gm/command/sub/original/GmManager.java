...
public class GmManager {
    ...
    public static void addServer(LiteralArgumentBuilder<CommandSourceStack> ogCommand, boolean useFullName) {
        ...
        ogCommand.then(Commands.literal(useFullName ? ACCEPT : ACCEPT_SHORT)
                .then(Commands.literal(useFullName ? GAME_MASTER : GAME_MASTER_SHORT)
                        .then(Commands.argument(ENTITY, EntityArgument.entity())
                                .executes(GmManager::acceptGm))));
        // 移除GM
        // /battleroyale original delete gm [entity]
        ogCommand.then(Commands.literal(useFullName ? DELETE : DELETE_SHORT)
                .then(Commands.literal(useFullName ? GAME_MASTER : GAME_MASTER_SHORT)
                        .then(Commands.argument(ENTITY, EntityArgument.entity())
                                .executes(GmManager::removeGm))));
    }

    /**
     * 授权GM
     */
    private static int acceptGm(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
     ...
     * 移除GM
     */
    private static int removeGm(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        Entity entity = EntityArgument.getEntity(context, ENTITY);
